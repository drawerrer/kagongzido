      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 76px)' }}>
        ...
        {showPhotoSheet && (
          <>
            ...
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#f3f3f3', borderRadius: '20px 20px 0 0', padding: '20px 20px 32px', zIndex: 201 }}>
              ...
              {[
                ...
                <button key={item.label} onClick={item.onClick} style={{ display: 'flex', alignItems: 'center', gap: 14, width: '100%', padding: '15px 4px', textAlign: 'left' }}>
                  {item.icon}
                  <span style={{ fontSize: 16, color: '#191F28' }}>{item.label}</span>
                </button>
              ))}
              <button onClick={() => setShowPhotoSheet(false)} style={{ marginTop: 16, width: '100%', height: 48, borderRadius: 12, fontSize: 15, fontWeight: 600, color: '#ffffff', background: '#252525', border: 'none' }}>
                닫기
              </button>
            </div>
          </>
